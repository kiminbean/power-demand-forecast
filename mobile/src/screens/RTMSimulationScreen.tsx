...
// RTM participants (fast-response resources)
...
// RTM volatility by hour (+10% ~ +40% vs DAM)
...
// RTM demand multiplier (smaller volumes)
...
export default function RTMSimulationScreen() {
  ...
  const volatility = RTM_VOLATILITY_MULTIPLIER[selectedHour] || 1.0;
  ...
  const generateSupplyBids = useCallback(() => {
    ...
    ourSegments.forEach((seg, idx) => {
      bids.push({
        ...
        price: Math.round(seg.price * volatility * 1.1),
        isOurs: true,
        status: 'pending',
      });
    });
  ...
  }, [supplyBids, demandBids, volatility]);
  ...
  const volatilityColor = volatility >= 1.3 ? colors.status.danger :
    volatility >= 1.2 ? colors.status.warning : colors.status.success;
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerText}>
          <View style={styles.headerTitleRow}>
            <Icon name="pulse" size={20} color={colors.status.warning} />
            <Text style={styles.headerTitle}>KPX 실시간시장 시뮬레이션</Text>
          </View>
      ...
      {/* Volatility Alert */}
      <View style={[styles.volatilityAlert, {
        backgroundColor: `${volatilityColor}15`,
        borderLeftColor: volatilityColor,
      }]}>
        <Icon name="pulse" size={18} color={volatilityColor} />
        <Text style={styles.volatilityText}>
          <Text style={{ fontWeight: 'bold' }}>RTM 가격 변동성: </Text>
          <Text style={{ color: volatilityColor, fontWeight: 'bold' }}>
            +{((volatility - 1) * 100).toFixed(0)}%
          </Text>
          <Text style={{ color: colors.text.muted }}>
            {volatility >= 1.3 ? ' (고변동 - 피크시간대)' :
             volatility >= 1.2 ? ' (중변동)' : ' (저변동 - 안정적)'}
          </Text>
        </Text>
      </View>
      ...
      <View style={styles.phaseContainer}>
        <View style={styles.phaseHeader}>
          <Icon name="time" size={18} color={colors.status.warning} />
      ...
      <View style={styles.actionRow}>
        {simulation.phase === 'idle' ? (
          <TouchableOpacity style={[styles.startButton, { backgroundColor: colors.status.warning }]} onPress={runSimulation}>
            <Icon name="play" size={20} color={colors.text.inverse} />
            <Text style={[styles.startButtonText, { color: colors.text.inverse }]}>시뮬레이션 시작</Text>
          </TouchableOpacity>
        ) : (
      ...
      {simulation.phase === 'complete' && simulation.clearingPrice && (
        <View style={styles.resultsContainer}>
          <View style={styles.resultCard}>
            ...
            <View style={[styles.resultMain, { backgroundColor: `${colors.status.warning}20` }]}>
              <Text style={[styles.clearingPrice, { color: colors.status.warning }]}>
                {simulation.clearingPrice.toFixed(1)}원
              </Text>
              <Text style={styles.resultLabel}>청산가격 (RTM SMP)</Text>
              <Text style={[styles.resultSubLabel, { color: colors.status.warning }]}>
                예측 대비 +{((simulation.clearingPrice / smpForecast.q50 - 1) * 100).toFixed(1)}%
              </Text>
            </View>
          ...
          <View style={[
            ...
            {simulation.ourAccepted > 0 ? (
              ...
              <View style={styles.failBadge}>
                ...
                <Text style={styles.failSubText}>입찰가격이 RTM 청산가격보다 높습니다</Text>
              </View>
            )}
          </View>
        </View>
      ...
      {/* Info Box */}
      <View style={[styles.infoBox, { backgroundColor: `${colors.status.warning}15`, borderColor: `${colors.status.warning}30` }]}>
        <Text style={[styles.infoTitle, { color: colors.status.warning }]}>KPX 실시간시장(RTM) 입찰 매칭 원리</Text>
        <Text style={styles.infoText}>
          • 거래 시점: 실제 전력 공급 5~15분 전{'\n'}
          • 가격 변동성: DAM 대비 10~40% 높은 변동성{'\n'}
          • 주요 참여자: ESS, 빠른 응답 가능한 발전원{'\n'}
          • 거래 목적: 재생에너지 변동 보상, 예비력 확보{'\n'}
          • 청산 방식: DAM과 동일한 Merit Order 방식
        </Text>
      </View>
